...
export default function MainPhoto () {
    ...
    return (
        <div className="container">
            <h4>Main Photo</h4>
            {
                loading ? <Preloader /> : (data && <img className="main-picture" src={data.urls.regular} alt="picture" /> ) 
            }
            {
                loading ? <Preloader /> : (data && <i id="material-icons" class="material-icons" >favorite_border</i> )
            }
            <br />
            <button className="btn" onClick={() => {getPhoto()}} >NEXT</button>
            {
                loading ? <Preloader /> : <HistoryPhoto  tempArrForHistory={history} />
            }
