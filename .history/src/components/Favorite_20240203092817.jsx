...
export default function Favorite() {
    ...
    if (!history || !history.length) {
        return <h5><span className="container grey-text text-darken-2">Информация 1 отсутствует</span></h5>
    }
    const likedObjects = history.filter(item => item.liked_by_user )
    // console.log(object);

    if (!likedObjects || !likedObjects.length) {
        return <h5 ><span className="container grey-text text-darken-2">Информация  2 отсутствует</span></h5>
    }
